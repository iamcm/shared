...
class EntityManager:
    ...
    def _sanitise_object(self, proptype, propvalue):
        if propvalue is None or proptype == unicode:
            return propvalue
        ...
        if proptype == list:
            ...
            for item in propvalue:
                item = self._sanitise_object(type(item), item)
                items.append(item)
    ...
    def find_raw(self, collectionname, objfilter=None, sort=None, count=False, skip=0, limit=0):
        """
        ...
        if count:
            ...
            if self.debug: 
                ...
                if skip:
                    kwargs += ' skip=%s,' % skip
                if limit:
                    kwargs += ' limit=%s,' % limit

                if len(kwargs)>0:
                    #strip the trailing comma
                    kwargs = kwargs[:-1]

                log_to_file("db.%s.find(%s, %s)" % (collectionname, objfilter, kwargs))

            return self.db[collectionname].find(objfilter, sort=sort, skip=skip, limit=limit)


    def find(self, collectionname, objfilter=None, sort=None, skip=0, limit=0):
        """
        Find all entries for a given collection and return them as a instances of the given entity
        """
        items = []
        for item in self.find_raw(collectionname, objfilter=objfilter, sort=sort, skip=skip, limit=limit):
        ...
        for word in searchterm.split(' '):
            ...
            if len(stems) > 0:
                ...
                for m, entity_count_dict in matches.items():
                    ...
                    if float(c) / float(len(stems)) * 100 > 70 \
                        or (len(m) < len(word)+2 and len(m) >= len(word) and float(c) / float(len(stems)) * 100 > 30):
